...
//		if (!_dragMode && _points[0].aboutEqual(point) && millis() - _touchStartTime > 50)
		if (!_dragMode && millis() - _touchStartTime > 20)
...
uint8_t FT6X36::getCachedTouches() {
  if (_touchUpdated) {
    _touchUpdated = false;  // Markiere als verarbeitet
    return _touches;        // Liefert gespeicherten Wert zurück
  }
  return 0;  // Kein neuer Touch
}
